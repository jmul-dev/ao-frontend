...
export default class DevelopmentBar extends PureComponent {
    render() {
        ...
        return (
            <div className="DevelopmentBar">
                ...
                <div
                    className={`status ${coreConnected ? "success" : "error"}`}
                >
                    core v{version}
                </div>
                <div className={`status ${!error ? "success" : "error"}`}>
                    graphql
                </div>
                <div
                    className={`status ${
                        this.props.isElectron ? "success" : "error"
                    }`}
                >
                    electron
                </div>
                <div
                    className={`status ${web3Available ? "success" : "error"}`}
                >
                    web3
                </div>
                {web3Connected ? (
                    <div className={`status success`}>
                        {getNetworkName(ethNetworkId)}
                        {this.props.latestBlockNumber
                            ? ` #${this.props.latestBlockNumber}`
                            : ""}
                    </div>
                ) : null}
                {web3Connected ? (
                    <div
                        className={`status ${
                            contractsInitialized ? "success" : "error"
                        }`}
                    >
                        ao-contracts
                    </div>
