...
export default class TeaserListing extends Component<Props> {
    constructor() {
        ...
        this._boundNavigationKeyListener = this._navigationKeyListener.bind(
            this
        );
    }
    componentDidMount() {
        this._getVideosRangeBasedOnActiveIndex(
            this.props.activeTeaserVideoIndex
        );
        document.addEventListener(
            "keydown",
            this._boundNavigationKeyListener,
            false
        );
    }
    ...
    componentWillReceiveProps(nextProps) {
        if (
            nextProps.activeTeaserVideoIndex !==
            this.props.activeTeaserVideoIndex
        ) {
            this._getVideosRangeBasedOnActiveIndex(
                nextProps.activeTeaserVideoIndex
    ...
    _getVideosRangeBasedOnActiveIndex = activeIndex => {
        ...
        if (activeIndex - 2 < 0) videosSlice.unshift(null);
        if (activeIndex - 2 < -1) videosSlice.unshift(null);
        this.setState({
            videosSlice
        });
    };
            ...
            <div className="TeaserListing">
                <div className="slider-track">
                    {videosSlice.map((video, index) => {
                        if (!video)
                            return <div key={`slide placeholder-${index}`} />;
                        const slideIsFullscreen =
                            activeFullscreenVideo &&
                            activeFullscreenVideo.id === video.id;
                        const slideIsActive = index === 2;
                        return (
                            <div
                                className={`slide ${
                                    index !== 2 ? "clickable" : "active"
                                } ${
                                    slideIsFullscreen ? "fullscreen" : ""
                                } ${video.recentlySeenHostsCount === 0 &&
                                    "no-recent-hosts"}`}
                                key={video.id}
                                style={
                                    index === 2
