...
const AccountRequired = connect((state) => ({
    ...
    return ethAddress ? children : (
        <PrimaryButton {...props} onClick={isElectron ? () => {window.chrome.ipcRenderer.send('open-metamask-popup')} : null}  disabled={!isElectron}>
            <img src={metamaskIconSrc} alt="Metamask icon" style={{height: 20, width: 'auto', marginRight: 8}} />
            {`Log in to access`}
        </PrimaryButton>
    )
})
