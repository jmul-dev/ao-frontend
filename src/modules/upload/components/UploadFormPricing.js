...
const PricingInputCard = withStyles(
    ...
    ({
        ...
        <ButtonBase
            focusRipple
            className={`pricing-card ${classes.base} ${
                selected ? "selected" : ""
            }`}
            onClick={onClick}
        >
            <div className={`pricing-icon ${iconClassName}`} />
...
const CustomPricingCard = withStyles(
    ({ palette, shape, shadows, spacing, transitions }) => ({
        base: {
    ...
    ({
        ...
        <ExpansionPanel
            className={`${classes.base} ${expanded ? "selected" : ""}`}
            expanded={expanded}
            onChange={onSelected}
            {...props}
        >
            ...
            <ExpansionPanelDetails>
                <div>
                    <Typography>
                        {"1. How much would you like to charge (AO/view)?"}
                    </Typography>
                    ...
                    <Typography>
                        {"2. Would you like to stake AO+, AO, or both?"}
                    </Typography>
                            ...
                            >
                                <TokenBalance
                                    baseAmount={new BigNumber(
                                        ...
                                        1 - stakePrimordialPercentage / 100
                                    )}
                                    decimals={1}
                                    includeAO={true}
                                    isPrimordial={false}
                    ...
                    <Typography>
                        {
                            "3. What percentage of the earnings would you like to make?"
                        }
                    </Typography>
                    <div className="profit-input-container indent">
                        ...
                        >
                            ...
                            <Typography
                                style={{ color: "#17BB59", marginLeft: "auto" }}
                            >
                                {profitSplitPercentage < 25
                                    ? "great exposure"
                                    : profitSplitPercentage < 50
                                    ? "good exposure"
                                    : profitSplitPercentage < 75
                                    ? "average exposure"
                                    : "less exposure"}
                            </Typography>
                        </div>
                        <Slider
                            ...
                            onChange={(event, value) =>
                                onChange(undefined, parseInt(value, 10))
                            }
                        />
                    </div>
                    {expanded ? (
...
class UploadFormPricing extends Component {
    ...
    _onCustomStakeInputChange = ({ value, denominationName }) => {
        const tokenAmountInBaseAo = fromDenominationValueToBase(
            value,
            denominationName
        );
        const contentSizeInBytes = contentSize(this.props.form.content);
        if (tokenAmountInBaseAo.gte(contentSizeInBytes)) {
            this.setState({
                customStakeInput: {
                    value,
            ...
            let customStake = fromBaseToHighestDenomination(
                this.props.form.stake
            );
            this.setState({
                customStakeInput: {
                    value: customStake.amount.toString(),
    ...
    _navForward = () => {
        ...
        if (
            wallet.networkTokenBalance.lt(form.networkTokensRequired) ||
            wallet.primordialTokenBalance.lt(form.primordialTokensRequired)
        ) {
                    ...
                    <Grid item xs={8} style={{ marginLeft: "auto" }}>
                            ...
                            <Grid container spacing={16}>
                                <Grid item xs={4}>
                                    <PricingInputCard
                                        ...
                                        stake={contentSizeInBytes}
                                        profitSplitPercentage={10}
                                        selected={form.pricingOption === 1}
                                        onClick={this._selectPricingOption.bind(
                                            this,
                                            1
                                        )}
                                    />
                        ...
                        <div className="custom-pricing gutter-bottom">
                            <CustomPricingCard
                                ...
                                stakePrimordialPercentage={
                                    form.stakePrimordialPercentage
                                }
                                onSelected={(_, expanded) =>
                                    expanded
                                        ? this._selectPricingOption(
                                              0,
                                              this.state.customStakeInput.baseValue.toNumber()
                                          )
                                        : this._selectPricingOption(1)
                                }
                                onChange={this._selectPricingOption.bind(
                                    this,
                                    0
                                )}
                                tokenInputs={this.state.customStakeInput}
                                onTokenInputChange={
                                    this._onCustomStakeInputChange
                                }
                            />
                        </div>
